# NOTE HACK-ed version of PPO trainer to accomoddate being able to drop ouputs in PPO backward step (length control thing)
...
class PPOTrainer(BaseTrainer):
            ...
            for j in range(len(query_batch)):
                if self.is_encoder_decoder:
                    ...
                    if attention_mask[j, 0] == 0:  # offset left padding
                        # HACK to get rid of too long outputs
                        try:
                            start += attention_mask[j, :].nonzero()[0]
                        except:
                            start =  -1
                    end = start + len(response_batch[j])
                # HACK to allow for null inputs
                if start>-1:
                    masks[j, :start] = 0
                    masks[j, end:] = 0
                else:
                    # just mask the whole thing
                    masks[j, :] = 0
    ...
    def compute_rewards(
        ...
        for score, logprob, ref_logprob, mask in zip(scores, logprobs, ref_logprobs, masks):
            ...
            # HACK to allow for hacky masking
            try:
