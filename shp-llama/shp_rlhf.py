...
def build_dataset(tokenizer, dataset_name="eli5", input_min_text_length=2, input_max_text_length=8):
    ...
    ds = ds.select(range(200000,300000))
    ...
    def tokenize(sample):
        ...
        for question in sample["history"]:
            query = question
            
            query = adjust_shp(query)
...
for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader)):
    ...
    if epoch==0:
        print("input to reward function")
        print(texts)
    pipe_outputs = sentiment_pipe(texts, **sent_kwargs)
    rewards = [torch.tensor(output[0]["score"])+2.7 for output in pipe_outputs]
