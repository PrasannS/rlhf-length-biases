...
def preproc_shp(example):
    ex = {}
    ex['question'] = example['history']
    if example['score_A']>example['score_B']:
        ex['response_j'] = example['human_ref_A']
        ex['response_k'] = example['human_ref_B']
    else:
        ex['response_k'] = example['human_ref_A']
        ex['response_j'] = example['human_ref_B']
    return ex
...
eval_dataset = load_dataset("stanfordnlp/SHP", split="test")
eval_dataset = eval_dataset.map(preproc_shp)
